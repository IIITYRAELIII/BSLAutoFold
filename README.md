...
- опционально сворачивает области, условия, циклы, блоки `Попытка` и директивы
  препроцессора;
- подсвечивает связанные управляющие слова, когда курсор установлен на одно из
  них:
  `Если` / `ИначеЕсли` / `Иначе` / `КонецЕсли`, циклы, `Попытка`, методы,
  области и директивы препроцессора;
- опционально сохраняет подсветку, когда курсор находится внутри тела
  конструкции;
...
| `bslAutoFold.highlightCurrentConstruct` | `true` | Подсвечивать связанные управляющие слова, когда курсор установлен на одно из них. |
| `bslAutoFold.highlightConstructWhenCursorInside` | `false` | Сохранять подсветку, когда курсор находится внутри тела конструкции. |
