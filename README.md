...
навигация по вложенным блокам напоминает редактор конфигуратора 1С.
...
- опционально сворачивает области, условия, циклы, блоки `Попытка` и директивы
  препроцессора;
- подсвечивает управляющие слова самой внутренней конструкции под курсором:
  `Если` / `ИначеЕсли` / `Иначе` / `КонецЕсли`, циклы, `Попытка`, методы,
  области и директивы препроцессора;
- поддерживает русские и английские ключевые слова BSL.
...
| `bslAutoFold.highlightCurrentConstruct` | `true` | Подсвечивать управляющие слова текущей конструкции. |
...
умолчанию. Подсветка использует цвета `editorBracketMatch.background` и
`editorBracketMatch.border` активной темы VS Code.

Перед автоматическим сворачиванием расширение сбрасывает восстановленное VS Code
состояние folding и затем применяет только включённые категории. Это не даёт
повторному `editor.fold` сворачивать родительские `#Области` вместо уже
свёрнутых методов.
